<%@ page language="java" import="com.questionnaire.survey.dao.AnswersheetDAO" pageEncoding="UTF-8" %>
...
<table class=table cellspacing="0" cellpadding="0" align="center">
    ...
    %>
    <tr>
        <td><%=answersheet.getAsId() %>
        </td>
        <td><%=answersheet.getAsPostdate() %>
        </td>
        <td><%=answersheet.getAsUserIp() %>
        </td>
        <td><a href=ShowSheet.jsp?asid=<%=answersheet.getAsId() %>&sid=<%=answersheet.getSurvey() %>>查看</a>|<a
                href="javascript:DelSheet(<%=answersheet.getAsId() %>,<%=answersheet.getSurvey() %>)">删除</a></td>
    </tr>
    <%} %>
    <tr>
