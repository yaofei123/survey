...
//            document.getElementById("qtype_dxjk").click();
                document.forms[0].qHead.value = "<%=question.getQHead().replace("\'","\\'").replace("\"","\\\"") %>";
...
//alert(qBody.length);
                ...
                for (i = 0; i < qBody.length ; i++) {
                    MoreAnswer(typecode.toUpperCase());
                }
                for (i = 0; i < qBody.length; i++) {
...
<table class=table cellspacing="0" cellpadding="0">
    ...
    <tr>
        ...
        <td><input name="Question_type" id="qtype_dx" type="radio" value="1" onClick="showType('dx');">单选题 <input
                name="Question_type" id=qtype_fx type="radio" value="2" onClick="showType('fx');">多选题 <input
                name="Question_type" id=qtype_dxjk type="radio" value="3" onClick="showType('dxjk');">带文字输入的单选题 <input
                name="Question_type" id=qtype_fxjk type="radio" value="4" onClick="showType('fxjk');">带文字输入的多选题 <input
                name="Question_type" id=qtype_wd type="radio" value="5" onClick="showType('wd');"> 问答题
        </td>
        <td></td>
    </tr>
    <tr>
...
<div id=dx style="display: none">
    <form name="form1"
          action="../servlet/QuestionManage.do?op=EditQuestion&type=1&qid=<%=request.getParameter("qid") %>"
          method="post" onSubmit="return SubQuestion();">
...
<div id=fx style="display: none">
    <form name="form1"
          action="../servlet/QuestionManage.do?op=EditQuestion&type=2&qid=<%=request.getParameter("qid") %>"
          method="post" onSubmit="return SubQuestion();">
        <input type="hidden" value=<%=request.getParameter("sid") %>  name="sid">
        <input type="hidden" value="" name="qBody" id="qBody">
        <input type="hidden" value="" name=qResult id="qResult">
...
<div id=dxjk style="display: none">
    <form name="form1"
          action="../servlet/QuestionManage.do?op=EditQuestion&type=3&qid=<%=request.getParameter("qid") %>"
          method="post" onSubmit="return SubQuestion();">
        <input type="hidden" value=<%=request.getParameter("sid") %>  name="sid">
        <input type="hidden" value="" name="qBody" id="qBody">
        <input type="hidden" value="" name=qResult id="qResult">
...
<div id=fxjk style="display: none">
    <form name="form1"
          action="../servlet/QuestionManage.do?op=EditQuestion&type=4&qid=<%=request.getParameter("qid") %>"
          method="post" onSubmit="return SubQuestion();">
        <input type="hidden" value=<%=request.getParameter("sid") %>  name="sid">
        <input type="hidden" value="" name="qBody" id="qBody">
        <input type="hidden" value="" name=qResult id="qResult">
...
<div id=wd style="display: none">
    <form name="form1"
          action="../servlet/QuestionManage.do?op=EditQuestion&type=5&qid=<%=request.getParameter("qid") %>"
          method="post" onSubmit="return SubQuestion();">
        <input type="hidden" value=<%=request.getParameter("sid") %>  name="sid">
        <input type="hidden" value="" name="qBody" id="qBody">
        <input type="hidden" value="" name=qResult id="qResult">
...
<%--//alert(qBody.length);--%>
            ...
            break;
        case 4:
            document.getElementById("qtype_fxjk").click();
            <%--document.forms[0].qHead.value = "<%=question.getQHead().replace("\'","\\'").replace("\"","\\\"") %>";--%>
            <%--var qBodys = "<%=question.getQBody().replace("\'","\\'").replace("\"","\\\"") %>";--%>
            <%--var qBody = qBodys.split("&$$&");--%>
<%--//alert(qBody.length);--%>
            ...
            break;
        case 5:
            document.getElementById("qtype_wd").click();
            <%--document.forms[0].qHead.value = "<%=question.getQHead().replace("\'","\\'").replace("\"","\\\"") %>";--%>
            break;
        default:
            alert("题目类型错误！");
